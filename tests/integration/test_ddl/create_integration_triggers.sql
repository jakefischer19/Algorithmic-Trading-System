...
-- Companies Trigger
DELIMITER $$
CREATE TRIGGER integration_companies_id_gen_trg
BEFORE INSERT ON integration_companies
FOR EACH ROW
BEGIN
    -- vars
    ...
    SET valid = 1;
	-- code
	SELECT COUNT(*) INTO temp FROM integration_companies WHERE symbol = NEW.symbol;
	IF (temp = 0) THEN
    	WHILE (valid = 1) DO
        	SELECT COUNT(*) INTO temp FROM integration_companies WHERE id = new_id;
        	IF (temp = 1) THEN
            	SET new_id = UUID_SHORT();
   			  ELSE
   		 		SET valid = 0;
...
-- Commodities Trigger
DELIMITER $$
CREATE TRIGGER integration_commodities_id_gen_trg
BEFORE INSERT ON integration_commodities
FOR EACH ROW
BEGIN
	-- vars
	DECLARE new_id BIGINT;
    ...
    SET valid = 1;
	-- code
	SELECT COUNT(*) INTO temp FROM integration_commodities WHERE Symbol = NEW.Symbol;
	IF (temp = 0) THEN
    	WHILE (valid = 1) DO
        	SELECT COUNT(*) INTO temp FROM integration_commodities WHERE id = new_id;
        	IF (temp = 1) THEN
            	SET new_id = UUID_SHORT();
   		 	  ELSE
   		 		SET valid = 0;
...
-- Index Trigger
DELIMITER $$
CREATE TRIGGER integration_indexes_id_gen_trg
BEFORE INSERT ON integration_indexes
FOR EACH ROW
BEGIN
	-- vars
	DECLARE new_id BIGINT;
    ...
    SET valid = 1;
	-- code
	SELECT COUNT(*) INTO temp FROM integration_indexes WHERE symbol = NEW.symbol;
	IF (temp = 0) THEN
    	WHILE (valid = 1) DO
        	SELECT COUNT(*) INTO temp FROM integration_indexes WHERE id = new_id;
        	IF (temp = 1) THEN
            	SET new_id = UUID_SHORT();
   		 	  ELSE
   		 		SET valid = 0;
...
-- Bonds Trigger
DELIMITER $$
CREATE TRIGGER integration_bonds_id_gen_trg
BEFORE INSERT ON integration_bonds
FOR EACH ROW
BEGIN
	-- vars
	DECLARE new_id BIGINT;
    ...
    SET valid = 1;
	-- code
	SELECT COUNT(*) INTO temp FROM integration_bonds WHERE treasuryName = NEW.treasuryName;
	IF (temp = 0) THEN
    	WHILE (valid = 1) DO
        	SELECT COUNT(*) INTO temp FROM integration_bonds WHERE bond_id = new_id;
        	IF (temp = 1) THEN
            	SET new_id = UUID_SHORT();
   		 	  ELSE
   		 		SET valid = 0;
...
*/
DELIMITER $$
CREATE TRIGGER integration_symbol_update_trg
AFTER UPDATE ON integration_companies
FOR EACH ROW
BEGIN
    -- vars
	DECLARE temp INTEGER;
	...
	-- code
    SELECT COUNT(*) INTO temp FROM integration_companies WHERE id = NEW.id;
    ...
    IF (temp > 0) THEN
        ...
        INSERT INTO integration_company_changelogs VALUES (company_id, change_date, OLD.companyName, new_name, name_changed, OLD.symbol, new_symbol, symbol_changed);
    ELSE
